...
**Version:** v2.5 (17 July 2026) — the mobile-zoom release, signed off on-device: solid paper-matched gridlines, phones always flush fit-width, 1×/2×/3× square-preserving phone zoom with sticky y-axis + floating zoom control, tightened gutters with inline unit captions, horizontal touch-panning fix, paper-matched "length"/"height" watermarks and view-button wording. v2.4 was the document-viewer redesign (fit-width Paper mode filling the chart column, Combined | Weight | Height selector); v2.3 introduced the Paper/Fit zoom modes; v2.2 was the viewport-fit sizing overhaul. Snapshots: `versions/paedplot_v2.5.html` (current) and `versions/paedplot_v2.4.html` (cut mid-arc, includes the zoom feature but not the watermark/wording changes).
...
```javascript
{
  ...
  zoomMode: 'paper'|'fit'|null,      // v2.3 — null = default 'paper'. Single-column is ALWAYS effectively 'paper' (July 2026); a stored 'fit' is ignored while narrow but kept for desktop
  zoomFactor: 1|2|3,                 // July 2026 — phone zoom (single-column only; desktop always renders 1×)
...
**Zoom modes (v2.3-v2.4):** two zoom modes toggled by a "Fit width"|"Fit page" button pair (internal ids/state values still `'paper'`/`'fit'` — only the button labels changed in v2.4) next to Combined/Weight/Height. **Fit width** ("paper" internally) fills the chart column width — `sheetW = clamp(availW − margins, SHEET_MIN_WIDTH 432, SHEET_MAX_WIDTH 1080)`, `pxSq = sheetW / xSquares` — mirroring the real charts, which all print at the same A4 sheet width with per-chart grid pitch. On single-column (phone) layouts the 432px floor is replaced by the per-range legibility floor (`MIN_PX_PER_SQUARE × xSquares`) and the right margin narrows 52→30 (`slimRightGutter()` / `effectiveMargins()` — every consumer of the CHART_MARGINS constants must go through the latter), so the sheet sits flush with the screen edge instead of overhanging ~145px behind a horizontal scroll (July 2026). The slim right gutter KEEPS its numeric y-labels — they're load-bearing (the left gutter switches kg→cm at the LEFT_WT_MAX/LEFT_HT_MIN cutoffs, so mid-range values are labelled on the right only) — and drops only the rotated kg/cm captions on that side. 1-4y (36 x-squares) still scrolls slightly on <~430px viewports where the 10px floor binds. Ranges with fewer x-squares (2-9y has 14) get proportionally bigger squares and more vertical page scroll. Since width now depends on the DOM, `fitSquareAndScale`'s paper branch runs the same margins↔scale fixed-point iteration as the fit branch (v2.3 used a fixed constant, `PAPER_PLOT_WIDTH = 720px`, with no iteration needed — superseded because it left dead space on wide columns and didn't grow with the window). **Fit page** ("fit") is the v2.2 viewport-fit behaviour below — **two-column (desktop) only** since July 2026. `getZoomMode()` resolves the effective mode: single-column is ALWAYS 'paper' (on-device testing showed flush fit-width strictly beats fit-page on phones; a stored 'fit' choice is ignored while narrow but not overwritten, so a desktop choice survives a narrow/widen round trip); two-column uses the explicit `chartState.zoomMode` if set, else 'paper'. Persisted as `zoomMode` in the session.

**Phone zoom 1×/2×/3× (July 2026):** on single-column layouts `syncZoomButtons()` swaps the Fit width|Fit page pair for a 1×|2×|3× trio (`btnZoom1-3` → `setZoomFactor`). The factor multiplies the CONVERGED flush square size outside the fixed-point loop in `fitSquareAndScale` (so margins can't dilute it — 2× is exactly 2× the 1× square; sqX===sqY holds by construction). Chrome (fonts/margins/dots) at zoom does NOT scale with the zoomed square: it uses the scale the 2-9y chart (ZOOM_REF_SQUARES = 14) converges to at flush width, so every range's 2×/3× view has identical label sizing (July 2026 — the flush scale varies with square count, and Nick picked 2-9y's as the reference; 1× keeps each range's own flush scale). Panning is normal scrolling: vertical page scroll + `.charts-stack` overflow-x, sticky age axis tracks as usual; the slim right gutter stays active. `setZoomFactor` preserves the viewport-centre point across the re-render (scroll positions recorded as fractions of canvas dims, restored scaled). `getZoomFactor()` returns 1 unless single-column. Persisted as `zoomFactor` (validated to [1,2,3] on load); reset to 1 by newPatient. Two frozen-pane companions (July 2026): **`#zoomFab`** — a fixed bottom-right floating 1×/2×/3× pill (phones only, synced alongside the inline trio in `syncZoomButtons`, lives inside `#chartsContainer` so it hides with the empty state, hidden in print) keeps zoom reachable while deep in a zoomed chart; **`updateStickyYAxis()`** — the vertical counterpart of the sticky age axis: whenever the sheet overflows `.charts-stack` horizontally in paper mode, it pixel-copies (`drawImage`) each visible canvas's left gutter (width published as `canvas._gutterPx` by both renderers) into an absolutely-positioned overlay (`#stickyYAxis`, inside `#pageCard`, which is `position: relative` for this — still NOT a scroll container) sitting outside the horizontal scroller, so kg/cm labels stay readable while panning; `pointer-events: none`, drop-shadow only while actually panned, called right after every `updateStickyAxis()` call.

**Gutter tightening (July 2026):** `CHART_MARGINS.left` 46→32 — the v2.4 rotated cm/kg caption columns were replaced by inline horizontal captions drawn at the head of each y-label run (foot of the run as fallback when the head is at the canvas edge or another run's label is too close) via the rewritten `drawGutterUnitCaption(ctx, entries, label, color, side, geom, s, allEntries, minY)`; `drawSingleChart`'s rotated left unit label was removed outright (its in-plot top-right panel unit label already covers it). Both label runs and captions share `drawYGutter`'s geometry, so the numbers+pad alone size the gutter and 32 clears 3-digit labels at any UI scale.
